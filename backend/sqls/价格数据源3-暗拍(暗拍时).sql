/*
文件: 价格数据源3-暗拍时出价深度.sql
指标: 暗拍车商出价|车主到手价|模型价|暗拍当时出价深度
业务: C1的C2B拍场
场景: 分析C2B拍场的暗拍当时出价深度
标签：暗拍|当时|出价深度|到手价|模型价
...
作者: 文国庆|杨悦芳
描述: 分析C2B拍场的暗拍当时出价深度|暗拍当时出价深度 = 暗拍车商出价 / 车主到手价模型价
*/
-- 暗拍当时出价深度 = 暗拍车商出价 / 车主到手价模型价（暗拍订单出价时）
with a as (
select
...
select 
    ...
    else 'null' end model_price_region -- 此处处理成null字符串是便于处理Excel中不可识别某列的单元格不为<>的问题
    ,sum(a.bid_amount) as bid_amount -- 暗拍车商出价
    ,sum(a.model_price) as model_price_sum -- 车主到手价模型价
    ,sum(a.bid_amount) / coalesce(sum(a.model_price), 0) as bid_depth_102
from a
left join c
on a.clue_id = c.clue_id
where rn=1 and a.model_price_region is not null
group by 1, 2, 3
