/*
文件: 价格数据源4-秒杀成交出价深度.sql
指标: 秒杀车商出价|车主心理底价|秒杀成交出价深度
业务: C1的C2B秒杀场
场景: 分析C2B秒杀场的秒杀成交出价深度
标签：秒杀|成交|出价深度|心理底价|模型价
...
作者: 文国庆|杨悦芳
描述: 分析C2B秒杀场的秒杀成交出价深度|秒杀成交出价深度 = 秒杀车商实付价 / 车主心理底价
*/
-- 秒杀成交出价深度 = 秒杀车商实付价 / 车主心理底价
with a as (
select
...
select 
    ...
    ,sum(b.deal_price) as deal_price_sum -- 秒杀车商实付价
    ,sum(a.bid_seller_price) as bid_seller_price -- 车主心理底价
    ,sum(b.deal_price) / coalesce(sum(a.bid_seller_price), 0) as bid_depth_103
from b -- b当主表
left join a
...
where a.model_bid_seller_price is not null 
group by 1, 2, 3
